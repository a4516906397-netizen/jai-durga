...
import { Home, Info, Box, Factory, Phone } from 'lucide-react';
...
const navItems = [
  ...
  { label: 'Infrastructure', path: PageRoute.INFRASTRUCTURE, icon: Factory },
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 border-b ${
        isScrolled || !isHome 
          ? 'bg-jdc-blue/90 backdrop-blur-xl py-3 border-white/5 shadow-2xl' 
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
        ...
        <div className="flex flex-col group cursor-default">
           <div className="flex items-center gap-3">
             <span className="text-white font-serif font-bold text-xl md:text-2xl tracking-wide group-hover:text-jdc-orange transition-colors duration-300">
               {COMPANY_NAME}
             </span>
             <span className={`hidden md:block px-2 py-0.5 border text-[9px] font-bold uppercase tracking-widest rounded transition-colors duration-300
                ${isScrolled || !isHome ? 'border-jdc-orange/50 text-jdc-orange bg-jdc-orange/10' : 'border-white/30 text-white/70 bg-white/10'}
             `}>
               ISO 9001
             </span>
           </div>
           <span className={`text-[10px] uppercase tracking-[0.3em] hidden md:block mt-1 transition-colors duration-300 ${isScrolled || !isHome ? 'text-slate-400' : 'text-slate-300'}`}>
